...
//
//import static ReviewHW.checkWin;
//import static ReviewHW.checkWin;
//import static ReviewHW.field;
//import static ru.gb.core.lesson2.S2.S2TicTacApp.*;
//
//public class ReviewHW {
//
//// Ярослав, проверил вашу работу, вижу вы поработали над переработкой метода проверки победы и
//// вы действительно проделали хорошую работу, тут замечаний быть не может, отлично.
//// Давайте, дополнительно, я постараюсь расписать соответствующие алгоритмы о которых я говорил
//// в конце семинара, применительно к разноразмерному полю (N на N клеток).
////
//// Начем с того, что мы должны написать универсальные методы проверки выигрыша, т е на вход,
//// как минимум, они должны получать текущую координату фишки (с которой начинается поиск
//// выигрышной комбинации) и значение выигрышной комбинации (последовательность фишек).
////
//// Таким образом, как я и описывал в концее лекции, по нашему алгоритму, по каждому
//// направлению строим проверки:
//
...
////endregion
//
////Далее мы перерабатываем наш универсальный метод checkWin. Теперь он универсален для
//// всех размерностей и последовательности выигрышей (начиная с 3),
////тут мы как раз вызываем соответствующие методы (описанные выше), проверяем выигрышные
//// комбинации, проходя по каждой клетке поля:
//
//    /**
...
////        return false;
////    }
////При этом методы gameChecks и main выглядят следующим образом, я добавил параметр
//// значение выигрышной комбинации (последовательность фишек),
...
//            if (!ReviewHW.SCANNER.next().equals("Y"))
//                break;
//        }
//    }
//// Отлично, а теперь немного о AI компьютера, я доработаю соответствующий метод,
//// теперь я буду проверять в первую очередь выигрышную последовательность для
//// компьютера, в случае
////если компьютер побеждает, ставлю фишку и завершаю работу. В случае, если нет,
//// проверяю выигрышную комбинацию человека (WIN - 1). Ну а если, ни человек, ни
//// компьютер не выигрывают - ставлю фишку, как и раньше в любое
...
//
//    //endregion
//// Вот и все. Да, работа действительно сложная, попробуйте скопировать данные методы, ввести
